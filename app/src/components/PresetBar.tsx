...
export default function PresetBar({ presets, disabled, onLoad, onSaveAs, onDelete }: Props) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <select
        className="min-w-40 rounded-md border border-slate-300 bg-white px-2 py-1.5 text-sm text-slate-900 focus:border-sky-500 focus:outline-none"
        value={selected}
      ...
      <button
        ...
        onClick={() => current && onLoad(current)}
      >
        载入
      </button>
      <button
